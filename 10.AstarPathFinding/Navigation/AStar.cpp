...
std::vector<Node*> AStar::FindPath(Node* startNode, Node* goalNode, std::vector<std::vector<int>>& grid)
{
	...
	// 비용 계산에 사용할 변수 값 설정
	// 이동할 때방향을 정함 - 그때그때 해도 되긴 하지만
	std::vector<Direction> directions =
	{
		// 하상우좌 (콘솔 왼쪽 위에서 시작해서 아래, 오른쪽으로 갈 확률이 높음) . 아무렇게나 해도 상관없긴 함
		{0, 1, 1.0f},
		{0, -1, 1.0f},
		{1, 0, 1.0f},
		{-1, 0, 1.0f},
	...
	while (!openList.empty())
	{
		...
		for (const Direction& direction : directions)
		{
			...
			// 이웃 노드가 열린 리스트에 있으면 더 좋은 비용이때만ㅇ=
			if (openListNode)
			{
				//비용 확인
				if (neighborNode->gCost < openListNode->gCost
					|| neighborNode->fCost == neighborNode->fCost)
				{
					//부모 노드 없데이트
					neighborNode->parentNode = neighborNode->parentNode;
					//비용 업데이트
			...
			//잠시 대기 (옵션)
			DisplayGrid(grid);
			WORD delay = static_cast<DWORD>(0.05f * 1000);
			Sleep(delay);
		}
	}
	return {};
